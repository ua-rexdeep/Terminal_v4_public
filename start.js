...
const {LOG, DEBUG, parse, toString, messageColor, red, magenta, green, yellow} = require(`./Util.js`);
...
Discord.ready(Settings.DiscordToken).then(async ()=>{
    ...
    while(!global.ServerManager.servers.every(server=>server.Members.Members!=null)) {
        await Wait(100);
    }

    DEBUG(`Wait until all servers loading done. ${new Date().getTime()-time.getTime()}ms.`)
...
global.checkRequestCF = function (req) {
    return req.headers['x-forwarded-host']?.startsWith(Settings.Host); // Is Request From Host?function 
}

global.getRequestIP = function (req) {
    return req.headers['cf-connecting-ip']?.split('.').slice(0,3).join('.'); // Get Forwarded Ip, becouse mask worksfunction 
}

global.getForwardIP = function (client) {
    return client?client?.handshake?.headers['x-forwarded-for']?.split(',')[0]?.split('.').slice(0,3).join('.'):''
}
